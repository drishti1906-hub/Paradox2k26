...
export const HUDPanel = ({ children, className = '', isDanger = false }) => (
  <motion.div 
    className={`relative bg-transparent border-[3px] p-4 overflow-hidden group transition-colors ${
      isDanger 
...
export const GlowButton = ({ children, onClick, isDanger = false, className = '' }) => (
  <motion.button
    onClick={onClick}
    className={`relative px-8 py-4 font-bold tracking-[0.2em] uppercase transition-all overflow-hidden ${
      isDanger 
...
export const MissionHUD = ({ timer = "18:42", timeInSeconds = 1122, score = "0" }) => {
  ...
  const timerColorClass = isCritical 
    ...
    : isWarning 
      ...
      : 'text-[#7C3AED] drop-shadow-[0_0_10px_rgba(124,58,237,0.5)]';

  return (
    <HUDPanel className="flex flex-col sm:flex-row justify-between items-center gap-6">
      <div className="flex flex-col">
        <span className="text-xs text-[#A7A0B8] tracking-widest uppercase mb-1">Current Mission</span>
        <span className="text-xl font-bold text-[#F8F7FF] tracking-widest uppercase">SYSTEM BREACH</span>
      </div>
      
      <div className="flex flex-col items-center">
        <span className="text-[10px] text-[#A7A0B8] tracking-widest uppercase mb-1">Time Remaining</span>
        <span className={`text-4xl font-mono font-black transition-colors duration-500 ${timerColorClass}`}>
          {timer}
        </span>
      </div>

      <div className="flex flex-col items-end">
        <span className="text-[10px] text-[#A7A0B8] tracking-widest uppercase mb-1">Score</span>
        <span className="text-2xl font-mono font-bold text-[#C084FC]">{score}</span>
        <div className="mt-2"><StatusIndicator status="ACTIVE" pulse /></div>
      </div>
    </HUDPanel>
  );
};

export const MainMissionPanel = ({ onEnterMission }) => (
  <HUDPanel className="flex flex-col h-full bg-[#160A27]/80">
    <div className="flex justify-between items-center mb-6">
      <div className="flex flex-col">
        <span className="text-xs text-[#A7A0B8] tracking-widest uppercase mb-1">CASE FILE 01</span>
        <h2 className="text-3xl font-black text-[#F8F7FF] uppercase tracking-widest">SYSTEM BREACH</h2>
      </div>
      <div className="flex items-center gap-2 text-[#EF4444] bg-[#EF4444]/10 px-3 py-1 rounded border border-[#EF4444]/30 animate-pulse">
        <AlertTriangle className="w-4 h-4" />
        <span className="text-[10px] font-bold tracking-widest uppercase">Anomaly Detected</span>
      </div>
    </div>

    <div className="flex-1 text-[#A7A0B8] text-sm leading-relaxed border-l-2 border-[#7C3AED]/50 pl-4 mb-8">
      "A critical system anomaly has been detected.<br/>
      Your team must investigate the available information<br/>
      and determine the most probable cause."
    </div>

    <div className="grid grid-cols-3 gap-4 mb-8 border-t border-b border-[#7C3AED]/20 py-4">
      <div className="flex flex-col">
        <span className="text-[10px] text-[#A7A0B8] tracking-widest uppercase mb-1">Mission Type</span>
        <span className="text-xs font-bold text-[#C084FC] uppercase tracking-wider">TECHNICAL INVESTIGATION</span>
      </div>
      <div className="flex flex-col">
        <span className="text-[10px] text-[#A7A0B8] tracking-widest uppercase mb-1">Difficulty</span>
        <span className="text-xs font-bold text-[#EF4444] uppercase tracking-wider">HIGH</span>
      </div>
      <div className="flex flex-col">
        <span className="text-[10px] text-[#A7A0B8] tracking-widest uppercase mb-1">Status</span>
        <span className="text-xs font-bold text-[#22C55E] uppercase tracking-wider">READY</span>
      </div>
    </div>

    <GlowButton onClick={onEnterMission} className="w-full text-lg">
      [ ENTER MISSION ] <ChevronRight className="w-5 h-5" />
    </GlowButton>
  </HUDPanel>
);
...
export const RoundTablePanel = ({ open = false }) => (
  <HUDPanel className="flex flex-col h-full bg-[#160A27]/80 justify-between">
    <div>
      <div className="flex items-center gap-3 mb-2">
        <Users className="text-[#A7A0B8] w-5 h-5" />
        <h3 className="text-lg font-bold text-[#A7A0B8] tracking-widest uppercase">ROUND TABLE</h3>
      </div>
      <p className="text-xs italic text-[#A7A0B8]/60 mb-6">"Someone among you is lying."</p>
    </div>
    
    <div className="flex flex-col items-center justify-center p-6 bg-black/40 border border-white/5 rounded text-center">
      {open ? <Unlock className="w-8 h-8 text-[#22C55E]/70 mb-3" /> : <Lock className="w-8 h-8 text-[#A7A0B8]/40 mb-3" />}
      <span className={`text-xs font-bold tracking-widest uppercase mb-1 ${open ? 'text-[#22C55E]' : 'text-[#EF4444]'}`}>{open ? 'OPEN' : 'LOCKED'}</span>
      <span className="text-[10px] text-[#A7A0B8]/60 uppercase tracking-widest">{open ? 'Discussion phase active.' : 'Discussion phase unavailable.'}</span>
    </div>

    <button className="mt-4 w-full py-3 bg-black/50 border border-white/10 text-[#A7A0B8]/50 text-xs font-bold tracking-[0.2em] uppercase cursor-default">
      {open ? '[ DISCUSSION OPEN ]' : '[ LOCKED ]'}
    </button>
  </HUDPanel>
);
